...
class SImgWatcher(object):
  def __init__(self,
               n_rows=3, n_cols=2, img_size=(128, 128), fig_size=3,
               vmin=None, vmax=None, cmap=plt.cm.gray_r):
    self.fig = plt.figure(figsize=((fig_size + 1) * n_cols - 1, (fig_size + 1) * n_rows - 1))
    self.vmin = vmin
    self.vmax = vmax
  ...
  def draw(self, *imgs):
    ...
    for col, imgs in zip(self.columns, imgs):
      for i, (im, cb) in enumerate(col):
        img = imgs[i]
        im.set_data(img)
